...
class SambanovaClient:
    ...
    def chat(self, model, messages, temperature=0.7, top_p=1.0, max_tokens=500):
        try:
            ...
            valid_models = ["Qwen2.5-72B-Instruct", "Meta-Llama-3.2-1B-Instruct"]  # Example model names
            if model not in valid_models:
                raise ValueError(f"Invalid model: {model}. Available models: {', '.join(valid_models)}")
